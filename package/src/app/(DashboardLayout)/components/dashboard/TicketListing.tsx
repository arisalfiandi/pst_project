  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box display="flex" alignItems="center" height="100%">
          <Chip
            label={statusObj[params.row.status === "menunggu" ? 1 : 0].status}
            color={statusObj[params.row.status === "menunggu" ? 1 : 0].color}
            sx={{
              height: 24,
              fontSize: "0.75rem",
              width: 100,
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Button
          variant="contained"
          color="success"
          onClick={() => dispatch(DeleteTicket(params.row.Id))}>
          <Typography sx={{ textAlign: "right" }}>Selesai</Typography>
        </Button>
      ),
      // session.status === 'authenticated' && (session.data.uid === 1099999 || session.data.role == 'admin') ? (
      //   <>
